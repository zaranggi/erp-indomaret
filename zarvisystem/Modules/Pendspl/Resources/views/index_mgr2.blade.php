    ...
    <div class="card">
                                ...
                                <div class="col-md-4">
                                    <select name="nik"  class="form-control form-control-uniform" data-fouc>
                                        @foreach($listspv as $r)
                                            @if($r->nik == $rspv->nik)
                                                <option value="{{$r->nik}}" selected> {{$r->nik}} - {{$r->name}} ( {{$r->name_department}} )</option>
                                            @else
        ...
        <div class="col-12">
            <div class="text-center">
                @if($no > 1)
                <form method="post" action="{{ url('/pendspl/dbm/app') }}">
                    @csrf
                    <input type="hidden" name="nik" value="{{$rspv->nik}}">
