    ...
    <div class="card">
        ...
        <div class="card-body border-left-3 border-left-blue-400 rounded-left-0">
            ...
            <fieldset class="mb-3">
                ...
                <form class="form-horizontal" role="form" method="POST" action="{{ url('/hisspl/dbm') }}">
        ...
        @foreach($listdep as $xx)
                <?php
            $namamgr = "";
            $namadep = $xx->name_department;

            foreach($depdetail[$xx->id] as $n){
                $namamgr = $n->name;
            }
        ...
        <div class="card-header header-elements-inline">
            <h5 class="card-title text-success font-weight-bold"> Manager : {{$namamgr}}   <br/></h5>
        </div>
        ...
        <div class="card-body border-left-3 border-left-blue-400 rounded-left-0">
            ...
            <table class="table datatable-button-html5-columns">
                ...
                $TOTAL_LEMBUR = 0;
                ?>
                @foreach($data[$xx->id] as $r)
                    <tr class="font-size-sm">
                        <td align="center">{{ $no }}</td>
                        <td class="text-left">{{ $r->NIK }} - {{  $r->NAME}}</td>
                        <td class="text-center">{{ $r->TOTAL_HARI }} Hari</td>

                        <td class="text-center">{{  $r->T_NORMAL}} Jam</td>
                        <td class="text-center">{{  $r->T_MERAH}} Jam</td>
                        <td class="text-center">{{  $r->TOTAL_LEMBUR}} Jam</td>

                    </tr>
                    <?php $no++;

                    $TOTAL_HARI = $TOTAL_HARI + $r->TOTAL_HARI;
                    $TOTAL_NORMAL = $TOTAL_NORMAL + $r->T_NORMAL;
                    $TOTAL_MERAH = $TOTAL_MERAH + $r->T_MERAH;
                    $TOTAL_LEMBUR = $TOTAL_LEMBUR + $r->TOTAL_LEMBUR;
                    ?>
                ...
                <tr class="font-size-md bg-info">
                    ...
                    <td class="text-center">{{ $TOTAL_HARI }} Hari</td>

                    <td class="text-center">{{  number_format($TOTAL_NORMAL) }} Jam</td>
                    <td class="text-center">{{  number_format($TOTAL_MERAH) }} Jam</td>
                    <td class="text-center">{{  number_format($TOTAL_LEMBUR) }} Jam</td>
                </tr>
